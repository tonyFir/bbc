...
public List getOrderDesc() {
    ...
    try {
        ...
        String sql = "select * from tb_student order by chinese  desc limit 0,3"; //定义查询数据表中前3条记录的SQL语句
    ...
    public static void main(String[] args) {
        ...
        System.out.println("查询语文成绩排在前3名的同学：");
        for(int i = 0;i<list.size();i++){
            Student student = (Student)list.get(i);
            System.out.println("编号为："+student.getId()+"，姓名："+student.getName()+"，性别："+student.getSex()+
                    "，语文成绩："+student.getChinese());
        }
    }
}
