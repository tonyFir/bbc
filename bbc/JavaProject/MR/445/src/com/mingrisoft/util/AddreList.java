...
public class AddreList extends JFrame implements ActionListener {
	...
	File file = new File("C://addressList.txt");	//创建文件对象
	/**
	...
	public static void main(String args[]) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddreList frame = new AddreList();		//创建本类对象
...
public void actionPerformed(ActionEvent e) {
	...
	try {
		...
		BufferedReader in = new BufferedReader(new FileReader(file));	//创建BufferedReader	对象
		String name = null;
		int number = 1;
