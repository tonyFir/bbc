...
public List getGradeDesc() {
    ...
    try {
        ...
        String sql = "select top 3 id ,name,english from tb_Grade order by english"; // 定义将图书表中信息进行排序的SQL语句
    ...
    public static void main(String[] args) {
        ...
        System.out.println("英语成绩排在后3的同学是：");
        for (int i = 0; i < list.size(); i++) {
            Grade grade = (Grade) list.get(i);
            System.out.println("编号：" + grade.getId() + " ，姓名："
                    + grade.getName() + "  ，英语成绩：" + grade.getEnglish());
        }
    }
